...
# 2. On SPARK2 (worker): ./startWorker.sh first
# 3. Then run this script on either machine (typically head)
#
  ...
  if [[ -n "${modelsJson:-}" ]]; then
    if echo "$modelsJson" | grep -q "\"max_model_len\":${maxModelLen}"; then
      ...
      if curl -fsS "http://127.0.0.1:${portBind}/v1/models" >/dev/null 2>&1; then
        ...
        logWarn "Since runCluster.sh uses --network host, host access *should* work."
      fi
      break
    else

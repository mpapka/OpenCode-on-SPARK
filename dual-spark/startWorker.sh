...
logInfo "Starting Ray worker via runCluster.sh (detached)..."
nohup bash "$SCRIPT_DIR/runCluster.sh" "$vllmImage" "$headIp" --worker "$hfCacheDir" \
...
logInfo "To shutdown: ./shutdown.sh"

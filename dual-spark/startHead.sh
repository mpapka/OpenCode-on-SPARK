...
logInfo "Starting Ray head via runCluster.sh (detached)..."
nohup bash "$SCRIPT_DIR/runCluster.sh" "$vllmImage" "$headIp" --head "$hfCacheDir" \
...
logInfo "Then use: ./shutdown.sh --all"

...
#
# Usage:
#   ./shutdown-distributed.sh              # Shutdown local node only
#   ./shutdown-distributed.sh --all        # Shutdown local + remote worker
#   ./shutdown-distributed.sh --worker     # Shutdown remote worker only
#   ./shutdown-distributed.sh --force      # Force kill without graceful drain
#
...
shutdown_local_processes() {
    ...
    # Kill any orphaned run_cluster.sh processes
